...
class OrderController:
    ...
    def accept_decline_order_driver(request):
        orders = Order.objects.filter(driver_id=None, order_status__in=["Finding driver."])
    ...
    def update_order_status_driver(request, order_id, action):
        ...
        if action in ["accept", "decline"]:
            ...
            return redirect("completeorcancelorderdriver")
    ...
    def update_order_status_vendor(request, order_id, action):
        ...
        try:
            ...
            # Handle case where the user is not a driver
            return redirect('acceptdeclineordervendor')  # or render an error message

        if action == "accept": # Assign the driver's ID to the order
            order.order_status = "Finding driver."

        elif action == "decline":
            order.order_status = "Declined by vendor."  # No change in driver order count
