function printServiceTypesTable(serviceTypes_Data)
    ...
    % Validate input
    if ~iscell(serviceTypes_Data) || size(serviceTypes_Data, 1) ~= 4
        error('Input serviceTypes_Data must be a cell array with 4 rows: [Car Wash Service, Probability, CDF, Range]');
    end
