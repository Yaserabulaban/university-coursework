function [statistics, car_Interval] = simulateCars(Number_Of_Cars, interArrival_Data, washBay1_data, washBay2_data, washBay3_data, serviceTypes_Data, rule_choice)
    ...
    for count = 1:Number_Of_Cars
        ...
        switch rule_choice
            case 1
                bay = mod(count - 1, 3) + 1;  % Sequential manner
            case 2
                % Find the idle wash bay or default to wash bay 1 if all are idle
        ...
        statistics{bay} = [statistics{bay}; {count, randServiceType, serviceTime, startTime, endTime, waitingTime, timeInSystem}];
    end
...
function printStatisticsTables(statistics)
    ...
    for j = 1:3
        ...
        fprintf('%-4s %-12s %-11s %-18s %-16s %-12s %-21s\n', 'n', 'RN for', 'Service', 'Time service', 'Time service', 'Waiting', 'Time spends in');
        fprintf('%-4s %-12s %-11s %-18s %-16s %-12s %-21s\n', '', 'service time', 'time', 'begins', 'ends', 'time', 'the system');
        for i = 1:size(statistics{j}, 1)
            fprintf('%-4d %-12d %-11d %-18d %-16d %-12d %-21d\n', ...
